// ==UserScript==
// @name         Open Apollo App LiveContainer2
// @version      1.1.0
// @author       AnthonyGress
// @match        *://*.reddit.com/*
...
// Note: Some search engines like DDG and Bing index redditmedia.com links
	...
	function openInLiveContainer() {
		const liveContainerUrl = convertToLiveContainerUrl(window.location.href);
		if (liveContainerUrl) {
			window.location.href = liveContainerUrl;
		}
	}

	function convertToLiveContainerUrl(urlString) {
		const match = urlString.match(regexRedditIDs);
		const url = new URL(urlString);

		if (url.pathname === '/' || url.pathname === '/?feed=home') {
			const apolloUrl = 'apollo://';
			const encoded = toBase64(apolloUrl);
		...
		if (match) {
			...
			if (url.pathname.includes('/comments/')) {
				apolloTarget = `apollo://${hostname}${url.pathname}`;
			} else {
				const sortingSuffixes = /\/(new|best|hot|top|rising)\/?$/;
				const cleanPath = url.pathname.replace(sortingSuffixes, '');
				apolloTarget = `apollo://${hostname}${cleanPath}`;
			}

			const encoded = toBase64(apolloTarget);
			return `livecontainer://open-url?url=${encoded}`;
		}
	...
	function processSearchResults() {
		const searchResults = document.querySelectorAll('a:not([data-livecontainer-listener-added])');
		searchResults.forEach(link => {
			const href = link.href;
			if (!href) return;
			
			if (href.match(regexRedditIDs)) {
				const liveContainerUrl = convertToLiveContainerUrl(href);
				if (liveContainerUrl) {
					link.setAttribute('data-livecontainer-listener-added', 'true');
